...
<div class="row">
    <form method="post" action="{{route('admin.attraction.update')}}" id="add-attraction-form" enctype="multipart/form-data">
        @csrf
            ...
            <div class="card mb-3 btn-reveal-trigger">
                <div class="card-header position-relative min-vh-30 mb-2">
                    <div class="cover-image">
                        ...
                        <div class="bg-holder rounded-3 rounded-bottom-0 cover" style="background-image:url({{asset('storage/'.$attraction->media->path)}});">
                            @else
                    ...
                    <div class="card-body bg-light">
                        <div class="row">
                            <div class="col-lg-6">
                                ...
                                <input class="form-control" id="title" type="text" value="{{$attraction->title}}" name="title" id="title" placeholder="Title in English" required />
                            ...
                            <div class="col-lg-6 right-text">
                                ...
                                <input class="form-control right-text" id="title_ar" value="{{getAttributeTranslated($attraction,'title')}}" type="text" placeholder="العنوان بالعربية" name="title_ar" id="title_ar" required />
                            ...
                            <div class="col-lg-6">
                                ...
                                <input class="form-control" id="stitle" type="text" value="{{$attraction->stitle}}" name="stitle" id="stitle" placeholder="Sub Title in English" required />
                                <h6 class="text-danger">{{$errors->first('name')}}</h6>
                            </div>
                            <div class="col-lg-6 right-text">
                                <label class="form-label" for="stitle_ar"><span class="text-danger">*</span> (AR) العنوان الفرعي</label>
                                <input class="form-control right-text" id="stitle_ar" value="{{getAttributeTranslated($attraction,'stitle')}}" type="text" placeholder="العنوان الفرعي بالعربية" name="stitle_ar" id="stitle_ar" required />
                                <h6 class="text-danger">{{$errors->first('email')}}</h6>
                            </div>
                            <div class="col-lg-6">
                                ...
                                <div class="input-group mb-3">
                                    <select class="form-select mb-3" name="code" id="code" style="max-width: 120px;">
                                        ...
                                        <option value="{{$code}}" @if($code==$attraction->code) selected @endif>{{$code}}</option>
                                        @endforeach
                                    </select>
                                    <input class="form-control mb-3" id="basic-form-phone" value="{{$attraction->phone}}" type="text" placeholder="501122333" name="phone" id="phone" required />
                                    <h6 class="text-danger">{{$errors->first('phone')}}</h6>
                                </div>
                            </div>
                            <div class="col-lg-6">
                                <label class="form-label" for="price">Price <span class="text-danger">*</span></label>
                                <input class="form-control" id="price" type="text" value="{{$attraction->price}}" name="price" id="price" placeholder="Starting Price" required />
                                <h6 class="text-danger">{{$errors->first('name')}}</h6>
                            </div>
                            <div class="col-lg-6">
                                ...
                                <select class="form-select mb-3" name="emirate" id="emirate" onchange="get_areas(this);" required>
                                    ...
                                    <option value="{{$emirate->id}}" @if($emirate->id == $attraction->emirates_id) selected @endif>{{$emirate->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-lg-6">
                                ...
                                <select class="form-select mb-3" name="area" id="area" required>
                                    ...
                                    <option value="{{$area->id}}" @if($area->id == $attraction->areas_id) selected @endif>{{$area->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-lg-6 mb-3">
                                <div class="min-vh-50">
                                    ...
                                    <textarea class="tinymce d-none" name="description">{!!$attraction->description!!}</textarea>
                                </div>
                            </div>
                            <div class="col-lg-6 mb-3">
                                <div class="min-vh-50 right-text">
                                    <label class="form-label" for="description_ar">(AR) وصف</label>
                                    <textarea class="tinymce d-none" name="description_ar">{!!getAttributeTranslated($attraction,'description')!!}</textarea>
                                </div>
                            </div>
                            <div class="col-lg-6 mb-3">
                            ...
                            <div class="col-lg-6">
                                ...
                                <div class="form-check form-switch">
                                    <input class="form-check-input" id="status" type="checkbox" name="status" @if($attraction->status) checked @endif />
                                    <label class="form-check-label" for="status">Published</label>
                                </div>
                            </div>
                            <div class="col-lg-12 mb-5">
                                <div class="row g-0" id="gallery-preview">
                                    @if($attraction->gallery)
                                    @foreach(json_decode($attraction->gallery) as $photo)
                                    <div class="col-3 p-1"><a class="glightbox" href="{{asset('storage/'.$photo)}}" data-gallery="gallery1" data-glightbox="data-glightbox"><img class="img-fluid rounded" src="{{asset('storage/'.$photo)}}" alt="..." /></a></div>
                                    @endforeach
                                    @endif
                                </div>
                            </div>
